...
export default function AiAnalyticsDashboard() {
  ...
  const hasAIData = data.ai.total_quizzes > 0;
  const hasManualData = data.manual.total_quizzes > 0;
  const hasAnyData = hasAIData || hasManualData;
  ...
  const chartDataAI = data.ai.recent
    ...
    .sort((a: any, b: any) => a.submitted_at.getTime() - b.submitted_at.getTime());
  ...
  const SubmissionCard = ({ item }: { item: any }) => (
    <div className="bg-[#2a2a2a] border border-gray-700 p-4 rounded-lg flex justify-between items-center text-white">
      <div>
        <p className="text-sm font-medium text-indigo-400">Submission #{item.id}</p>
  ...
  if (!hasAnyData) {
    return (
      <div className="mt-10 text-sm text-gray-400 bg-[#1f1f1f] border border-gray-700 rounded-xl p-6">
        <p className="mb-2 text-white font-medium">Analytics</p>
        <p>You haven’t taken any quizzes yet. Once you do, your performance will appear here as charts and stats.</p>
      </div>
    );
  }
      ...
      >
        {hasAIData && (
          <div className="bg-[#1f1f1f] p-6 rounded-xl shadow-md border border-gray-700">
            <h3 className="text-lg font-semibold text-white mb-2">AI Quiz Stats</h3>
            <p className="text-sm text-gray-400 mb-1">
              Total Quizzes: <span className="text-white">{data.ai.total_quizzes}</span>
            </p>
            <p className="text-sm text-gray-400">
              Average Score: <span className="text-white">{data.ai.average_score}%</span>
            </p>
          </div>
        )}
        ...
        {hasManualData && (
          <div className="bg-[#1f1f1f] p-6 rounded-xl shadow-md border border-gray-700">
            <h3 className="text-lg font-semibold text-white mb-2">Manual Quiz Stats</h3>
            <p className="text-sm text-gray-400 mb-1">
              Total Quizzes:{" "}
